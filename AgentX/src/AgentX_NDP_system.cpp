...
class AgentXNDPSystem final : public IMemorySystem, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IMemorySystem, AgentXNDPSystem, "AgentX-NDP", "AgentX-NDP memory system.");
  ...
  public:
    ...
    int s_num_pim_mac_requests = 0;
    int s_num_pim_write_requests = 0;
  ...
  public:
    void init() override { 
      ...
      register_stat(s_num_pim_mac_requests).name("total_num_pim_mac_requests");
      register_stat(s_num_pim_write_requests).name("total_num_pim_write_requests");
    ...
    bool send(Request req) override {
      ...
      if (is_success) {
        switch (req.type_id) {
          ...
          case Request::Type::PIM_MACAB: {
            s_num_pim_mac_requests++;
            break;
          }
          case Request::Type::PIM_WRAB: {
            s_num_pim_write_requests++;
            break;
          }
