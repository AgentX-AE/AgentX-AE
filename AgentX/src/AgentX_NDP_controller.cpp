...
class AgentXNDPController final : public IDRAMController, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAMController,AgentXNDPController,"AgentX-NDP","AgentX-NDP controller (based on Generic-lite + PIM buffer).");
  private:
    std::deque<Request> pending;      // Completed reads waiting for callback after RL

    ReqBuffer m_active_buffer;        // Requests being activated/served (highest priority)
    ReqBuffer m_priority_buffer;      // High-priority maintenance requests (refresh, etc.)
    ReqBuffer m_pim_buffer;           // PIM requests buffer (PIM_MAC / PIM_Write / PIM_BARRIER)
    ReqBuffer m_read_buffer;          // Host read buffer
    ReqBuffer m_write_buffer;         // Host write buffer
  ...
  public:
    ...
    bool send(Request& req) override {
      ...
      switch (req.type_id) {
        ...
        case Request::Type::PIM_MACAB:      is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_WRAB:       is_success = m_pim_buffer.enqueue(req);   break;
