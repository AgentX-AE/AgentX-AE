...
class NDPLoadStoreTrace : public IFrontEnd, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IFrontEnd, NDPLoadStoreTrace, "NDPLoadStoreTrace", "PIM/Load/Store memory address trace.")
    ...
    void tick() override {
      ...
      while(!req_full && !is_finished()) {
        ...
        switch (t.req_type) {
          ...
          case  4: request_sent = m_memory_system->send({t.addr, Request::Type::PIM_MACAB}); break;
          case  5: request_sent = m_memory_system->send({t.addr, Request::Type::PIM_WRAB}); break;
          case  6: request_sent = m_memory_system->send({t.addr, Request::Type::PIM_BARRIER}); break;
          default:;
        }
        if (request_sent) {
      ...
      while (std::getline(trace_file, line)) {
       ...
       // printf("Debug: Token[0]: %s, Token[1]: %s\n", tokens[0].c_str(), tokens[1].c_str());
        ...
        if (tokens[0] == "LD") {
          ...
          req_type = 1;
        } else if (tokens[0] == "PIM_MACAB") {
          req_type = 4;
        } else if (tokens[0] == "PIM_WRAB") {
          req_type = 5;
