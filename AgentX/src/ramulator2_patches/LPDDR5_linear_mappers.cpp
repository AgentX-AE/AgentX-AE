...
class LinearMapperBase : public IAddrMapper {
  ...
  protected:
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) {
      ...
      // Determine where are the row and col bits for LPDDR5Mapper and RoBaRaCoCh
      try {
...
class LPDDR5Mapper final : public LinearMapperBase, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IAddrMapper, LPDDR5Mapper, "LPDDR5", "Applies a trival mapping to the address.");
  ...
  public:
    ...
    void apply(Request& req) override {
      ...
      req.addr_vec[5] = slice_lower_bits(addr, m_addr_bits[5]); // Co
      req.addr_vec[4] = slice_lower_bits(addr, m_addr_bits[4]); // Ro
      req.addr_vec[3] = slice_lower_bits(addr, m_addr_bits[3]); // Ba
